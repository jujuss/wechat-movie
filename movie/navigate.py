...
def navigate():
    ...
    if start_pos and end_pos:
        start_pos_long, start_pos_lat = \
            [float(i) for i in start_pos.split(',')]
        end_pos_long, end_pos_lat = [float(i) for i in end_pos.split(',')]
        end_name = request.args.get('end_name')
        baidu_geocoding_url = '%s?ak=%s&location=%s&output=%s' % (
            config.baidu_map_geocoding_api,
            config.baidu_ak,
            '%s,%s' % (start_pos_lat, start_pos_long),
            'json')
        ...
        if curr_pos_info['status'] == 0:
            start_name = \
                curr_pos_info['result'].get('formatted_address', '我的位置')
